...
│  ├─ dy.png               抖音爆款视频作品图
│  ├─ IP.jpg               IP 衍生周边作品图
│  ├─ website.png          个人简历网页开发作品图
│  ├─ tw.jpg               公众号推文排版作品图
│  ├─ background.jpg       整页背景图（位置、透明度可调）
│  ├─ favicon.png          网站图标
...
2. **作品集图片**：作品集与项目卡的图片已全部替换为真实作品图（`four seasons.jpg`、`dy.png`、`IP.jpg`、`website.png`、`tw.jpg`）。以后换图，直接覆盖 `images/` 里的同名文件即可，不需要改代码。
...
- **中英文切换**：右上角 EN / 中文 按钮切换全站语言，选择会被记住。
- **背景图调整**：编辑 `css/style.css` 顶部的 `--bg-image-opacity`（透明度，0-1）和 `--bg-image-position`（位置，如 `center`、`top`、`50% 30%`）即可，无需改其他代码。
